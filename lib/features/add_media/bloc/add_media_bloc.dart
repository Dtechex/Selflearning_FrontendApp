...
class AddMediaBloc extends Bloc<AddMediaEvent, AddMediaInitial> {
  ...
  _onSubmitButtonEvent(SubmitButtonEvent event, Emitter<AddMediaInitial> emit)async {
   ...
   if(event.whichResources==2){
     ...
     await  AddMediaRepo.addQuickAddwithResources(imagePath: state.selectedFilepath,title: event.title??'Untitled').then((value) {
         emit(state.copyWith(apiState: ApiState.submitted,wichResources: 0));
     });
   }else {
     await  AddMediaRepo.addResources(imagePath: state.selectedFilepath, resourceId:  event.rootId??'',).then((value) {
       if (value != null) {
