...
class SearchSubCategoryRepo {
  ...
  static Future<List<SearchCategoryModel>> searchSubCategories(String? query, String? rootId) async {
    ...
    Response res = await _dio.get("https://virtuosocity.com/web/category/search?keyword=$query",
    queryParameters: {'categoryId': rootId},

        options: Options(
          headers: {
            'Authorization': 'Bearer ${SharedPref.getUserToken()}', // Add the bearer token header
          },
        )
    );
